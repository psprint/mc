...
widget_focus (Widget * w)
{
    ...
    if (WIDGET (g->current->data) != w)
    {
        ...
        pymc_set_current_widget(WIDGET(g->current));

    }
    else if (!widget_get_state (w, WST_FOCUSED)) {
        widget_do_focus (w, TRUE);
        pymc_set_current_widget(WIDGET(w));
    }
    //printf("############### SET TO %p ###############",w);
    PyRun_SimpleString(
...
"            str(olist._count) + ' ≈ ˙˙˙ from °len()° —→ ≈ ' + \\
             str(len(olist)) + ' ≈ \\n')\n"
"        myfile.write('«FILE1» —→ ≈ '+str(olist[0].filename[1:] if \\\n"
"            (olist[0].filename != None) else None) + ' ≈\\n')\n"
"        myfile.write('«FILE2» —→ ≈ '+str(olist[1].filename if \\\n"
"            (olist[1].filename != None) else None) + ' ≈\\n\\n')");
}

...
edit_window_list (const WDialog * h)
{
    ...
    for (w = g->widgets; w != NULL; w = g_list_next (w)) {
        fprintf(F, "2) w->data °°el from g->widgets°° —→ %p\n", CONST_WIDGET(w->data));
        fflush(F);
...
edit_files (const GList * files)
{
    ...
    if (ok) {
        pymc_set_frontent_dialog(edit_dlg);
#ifdef ENABLE_PYTHON
    if(!pymc_init_python()) {
        fprintf(stderr, "Couldn't initialize the python interpreter.");
    }
    pymc_run_python_init_code();
#endif
